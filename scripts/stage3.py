...
print(f"{'src->dst':28s} layer | cos_pred cos_offset cos_ident cos_mean | role_rank: pred offset ident (of {len(roles)})")
res = {}
for src, dst in pairs:
    ...
    for l in range(0, L, a.step):
        ...
        ev = operate.holdout_eval(S, O, groups, l, src, dst, cands=C, dst_idx=ri[dst], ridge=a.ridge, low_rank=a.rank)
        res.setdefault(f"{src}->{dst}", {})[l] = ev
        if best is None or ev["cos_pred"] > best[1]["cos_pred"]:
            best = (l, ev)
    l, ev = best
    print(f"{src+'->'+dst:28s} {l:5d} | {ev['cos_pred']:8.3f} {ev['cos_offset']:10.3f} {ev['cos_identity']:9.3f} {ev['cos_mean']:8.3f} | {ev['role_rank']:10.1f} {ev['role_rank_offset']:6.1f} {ev['role_rank_identity']:5.1f}")
summ = {k: float(np.mean([res[p][l][k] for p in res for l in res[p]])) for k in ("role_rank", "role_rank_offset", "role_rank_identity")}
best_l = {k: float(np.mean([min(res[p].values(), key=lambda e: e[k])[k] for p in res])) for k in ("role_rank", "role_rank_offset", "role_rank_identity")}
